...
dfm_data = dfm(data_corpus_irishbudget2010)
dfm_data_group = dfm(data_corpus_irishbudget2010, groups = "party")
tokens_data = tokens(data_corpus_irishbudget2010)
dfm_party = dfm(tokens_data, groups = "party", remove_punct = TRUE, remove_numbers = TRUE, remove = stopwords("english"))
colnames(docvars(dfm)) ## "year"   "debate" "number" "foren"  "name"   "party" 
docvars(dfm)

example(dotchart)


posDict <- dictionary(list(populism = c("eliit", "consensus", "undemocratic", "referend", "corrupt", "propagand", "politici", "deceit", "deceiv", "betray", "shame", "scandal", "truth", "dishonest", "establishm", "ruling")))

dfm_posDict = dfm(posDict)

# Dic2 = dictionary(list(populism = c("eliit*", "consensus*", "undemocratic*", "referend*", "corrupt*", "propagand*", "politici*", "*deceit*", "*deceiv*","*betray*", "shame*", "scandal*",
# "truth*", "dishonest*", "establishm*", "ruling*")))

dfm_dictionary = dfm(posDict)


dfm(dfm_data_group , dictionary = posDict)
## this is not grouped but are all populistic words for each speacher
## dfm(dfm_data, dictionary = posDict)
dfm(dfm_party, dictionary = posDict)


#thesaurus
thes_group = dfm(dfm_data_group, thesaurus = posDict)
docvars(thes_group)
thes_party = dfm(dfm_party, thesaurus = posDict)
docvars(thes_party)
